...
const SAP_URL = 'http://AZKTLDS5CP.kcloud.com:8000/sap/bc/srt/scs/sap/zpe_login_sd?sap-client=100';
const SAP_AUTH = {
  username: 'K901564',
  password: '06-Aug-030'
};
...
router.post('/login', async (req, res) => {
  ...
  if (!EMPLOYEE_ID || !PASSWORD) {
    return res.status(400).json({ success: false, message: 'Missing EmployeeId or password' });
  }
  ...
  <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"
      ...
      <soapenv:Header/>
      <soapenv:Body>
        <urn:ZEMPLOYEE_LOGIN_FM>
          <IV_EMPLOYEE_ID>${EMPLOYEE_ID}</IV_EMPLOYEE_ID>
